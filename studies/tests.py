...
class ProgressTestCase(MiscProposalTestCase):
    ...
    def test_progress_double(self):
        ...
        self.assertEqual(STUDY_PROGRESS_START, get_study_progress(s1))
        self.assertEqual(
            STUDY_PROGRESS_START + 1 / 2 * STUDY_PROGRESS_TOTAL, get_study_progress(s2)
        )
    ...
    def test_progress_triple(self):
        ...
        self.assertEqual(
            int(STUDY_PROGRESS_START + 1 / 3 * STUDY_PROGRESS_TOTAL),
            get_study_progress(s2),
        )
        self.assertEqual(
            int(STUDY_PROGRESS_START + 2 / 3 * STUDY_PROGRESS_TOTAL),
            get_study_progress(s3),
        )
