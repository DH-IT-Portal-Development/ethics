...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open('output.csv', 'wb') as csvfile:
            ...
            header = ['titel studie', 'type(s) studie', 'indiener', 'eindverantwoordelijke', 'ingediend op', 'route', 'afhandeling', 'afgehandeld op']
            ...
            for proposal in Proposal.objects.all():
                if proposal.status >= Proposal.DECISION_MADE:
                    ...
                    row = [proposal.title,
                           study_types_to_string(study_types),
...
def study_types_to_string(study_types):
    result = []
