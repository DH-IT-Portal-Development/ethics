...
class ProposalURL(object):
    def __init__(self, title, margin, url=None):
        self.title = title
        self.margin = margin
        self.url = url
...
def session_urls(study):
    ...
    urls.append(tasks_url)
        ...
        # session = study.proposal.current_session()

        # if session:
            # urls.append(ProposalURL(url=reverse('tasks:start', args=(session.pk,)), title=_('Algemene informatie over de studie'), margin=0))
            # TODO: fix below
