...
class MiscProposalTestCase(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            username="test0101", email="test@test.com", password="secret"
        )
        self.secretary = User.objects.create_user(
            username="secretarytest010101", email="test@test.nl", password="more_secret"
        )
        Group.objects.get(name=settings.GROUP_PRIMARY_SECRETARY).user_set.add(
            self.secretary
        )
...
class ProposalTestCase(MiscProposalTestCase):
    def test_reference_number(self):
        ...
        user2 = User.objects.create_user(
            username="test0102", email="test@test.com", password="secret"
        )
        p3 = Proposal.objects.create(
            title="p3",
            reference_number=generate_ref_number(),
            date_start=datetime.now(),
            created_by=user2,
    ...
    def test_check_local_facilities(self):
        ...
        self.assertSetEqual(
            facs[Recruitment._meta.verbose_name],
            set([r.description for r in recruitments]),
        )
        ...
        self.assertSetEqual(
            facs[Registration._meta.verbose_name],
...
class ProposalsViewTestCase(MiscProposalTestCase):
    ...
    def test_supervisor(self):
        ...
        self.assertSequenceEqual(
            [response.data["items"][0]["pk"], response.data["items"][1]["pk"]],
            [self.p1.pk, p2.pk],
        )
