 ...
 */
(function() {
  var Match, calculate_operations, consecutive_where, create_index, diff, find_match, find_matching_blocks, html_to_tokens, is_end_of_tag, is_start_of_tag, is_tag, is_whitespace, isnt_tag, op_map, recursively_find_matching_blocks, render_after_operations, render_before_operations, render_operations, wrap;
  ...
  Match = (function() {
    function Match(_at_start_in_before, _at_start_in_after, _at_length) {
      this.start_in_before = _at_start_in_before;
      this.start_in_after = _at_start_in_after;
  ...
  render_before_operations = function(before_tokens, after_tokens, operations) {
    var action, op, rendering, _i, _len;
    rendering = '';
    for (_i = 0, _len = operations.length; _i < _len; _i++) {
      op = operations[_i];
      action = op.action;
      ...
      rendering += op_map[action](op, before_tokens, after_tokens);
    }
    return rendering;
  };

  render_after_operations = function(before_tokens, after_tokens, operations) {
    var action, op, rendering, _i, _len;
    rendering = '';
    for (_i = 0, _len = operations.length; _i < _len; _i++) {
      op = operations[_i];
  ...
  if (typeof define === 'function') {
    ...
    module.exports = diff;
    module.html_to_tokens = html_to_tokens;
    module.calculate_operations = calculate_operations;
    module.render_operations = render_operations;
  } else {
    this.htmldiff = diff;
    this.html_to_tokens = html_to_tokens;
    this.calculate_operations = calculate_operations;
    this.render_operations = render_operations;
    this.render_before_operations = render_before_operations;
    this.render_after_operations = render_after_operations;
  }
