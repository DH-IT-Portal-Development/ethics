...
class ConditionalModelForm(forms.ModelForm):
    ...
    def check_dependency_list(
        self, cleaned_data, f1, f2, f1_value_list, error_message=""
    ):
        is_required = False
        if not error_message:
            error_message = _("Dit veld is verplicht.")
    ...
    def check_dependency_singular(
        self, cleaned_data, f1, f1_field, f2, error_message=""
    ):
        is_required = False
        if not error_message:
            error_message = _("Dit veld is verplicht.")
        if cleaned_data.get(f1):
            if getattr(cleaned_data.get(f1), f1_field) and is_empty(
                cleaned_data.get(f2)
            ):
                is_required = True
                self.add_error(
                    f2, forms.ValidationError(error_message, code="required")
                )
        return is_required

    def check_dependency_multiple(
        self, cleaned_data, f1, f1_field, f2, error_message=""
    ):
        is_required = False
        if not error_message:
